...
User (has-many -> / belongs to <-) Post 이러한 관계가 있다면 1:1

User와 Profile은 1:1 관계.

has many / belongs to 이 중 아무것도 아니라면 M:N을 만들어라.

???????

그러니까 has many 는 1:N, belongs to는 1:1 인 것인가????



only one

여하튼 필터링에 모두 걸러지지 않으면 무조건 M:N
...
User모델을 우리가 조작할 순 없다. 그러니 새로 커스터마이징.

생성 폼도 커스터마이징 가능은 하다. 추후에 할 것이다.

### -1 클래스 상속을 이용한 models

Python의 상속을 이용해서. 할 것.

지난시간에 User가 누굴 상속했는지 봤었다. models에서 abstractuser, abstractbaseuser
...
내가 할로우를 하는 사람, 나를 팔로우 하는 사람 구분.

related_name이 필수다.

related_name은 목적어가 주어를 어떻게 부를까를 정의하는 것으로 이름 짓는다.
...
Default auth user가 바뀌었다. 원래는 django가 가지고 있는 user를 썼지만, 이젠 우린 user를 우리가 만든 애를 쓸 것이다.
...
# 기본값으로 "auth.User" 로 되어있었다. 다만 
```



예전의 흔적 있어서 DB 날린다. 기본값으로 만든 User와 충돌을 피하고자 한다.

accounts의 migrate도 날려야하는데 애초에 만든게 없어서 노상관.
...
여기에 소스 살피면 얘는 장고의 User를 받는다. 하지만 우린 커스터마이징
...
근데 강사님은 django에게 건의해 볼 생각. 

User를 고정해서 받지 않고 언제든디 커스터마이징한게 자동으로 들어가도록.



**forms.py (accounts) - 디버깅**
...
유저의 아이디를 string으로 받아와서 동적으로 처리하자.
...
**views.py (posts) - like 함수 추가**

하나의 액션으로 팔로우 언팔로우를 한꺼번에 구현.

우선 팔로우할 목적이 되는 사람을 가져온다.

그냥 가져올 수 없으니 getusermodel, pk 를 이용해서 팔로우할 대상 가져옴.
...
**profile.html (accounts) - 팔로우버튼 수정**
...
로그인 안 한 상태로 팔로우 누르면 에러.

이를 방지하기 위해 accounts에서 decorator login django a
...
네이버 같은 대기업은 모든 DB를 삭제하지 않고( 말만 삭제 했다고 한다.) 그대로 남겨둔다. 용량을 많이 먹겠지만 대기업이니 가능.
...
일단 id 역순. 즉 최신 글부터 출력하게 끔 코드 구현하라.
