...
mellicious python package 검색하면 절대 다운 받지 말아야할 목록 나옴
...
대문자로 pip  받으려 했더니 이미 깔려있덴다. 별 차이 없는듯.
...
shell_plus는 너무 많으면 중간에 출력하다 생략한다.
...
이름 바꾸고 싶어? resort_set 싫다면 models를 수정해야 한다.
...
font awesome을 통해 하트로 출력





template request에서는 그냥 user 가능. views에서는 request.user 써야 하는게 맞다.
