...
JS는 고수준의 런타임으로 만들어진 언어가 아니다.

JS 몇가지 특징. 싱글 스레드. 못 들은 걸로 해라.

무를 하자고 운동하고자 하노 만든게 node JS.

브라우저 말고 node라는 환경에서 돌아가는 JS를 말한다.
...
react나 많은 모듈을 이걸 통해 설치가 가능하다. python에서 pip와 비슷한 느낌.



질문 : node에서 만든 JS는 브라우저에 그대로 복붙하여 사용 가능?

alert 등은 브라우저에서만 가능한 것 처럼. node js에서만 작성한 js는 alert는 동작안하고
...
그러면 package.json 파일이 생겨있다.

무슨 모듈을 확인하는 용도의 파일이라고 보면 된다??



**package.json**

dependencies가 뭘 정의하고...

가운데 lodash가 깔려있는게 보인다.

패키지 매니저 느낌을 담고 있는 파일
...
air bnb 에도 obejct 선언할 때 항상 마지막에 , 붙인다.
...
어느 집단이 표준을 정해주면 좋겠다해서, 에크마인터네셔널

에크마스크립트는 ES5에 크게 뜨고 몇년 정채되었다가, ES2015부터 급 발전 이룸.



bash에서 5 누르고 tab만해도 바로 js 파일 실행 가능
...
그래서 중요한 변수는 const를 사용하는 것을 권장하고 있다.
...
html 요소를 트리화 해서 모델? 시킨 것이 다큐먼트 모델이라고 한다.
...
아하 html을 트리처럼 짤 수 있다. 



자바스크립트 오브젝트로 표현...? 파이썬의 딕셔너리 처럼 표현!

background-color 라고 우린 쓰지만, 오브젝트 안에는 backgroundColor!@@#%@#$ 로 키와 값으로 될 듯.



트리로 짜고 자바스크립트 오브젝트에 불과하다.
...
document.body. 쓰면 여러가지 쓸 수 있는게 보인다.



요것들을 js로 표현한다면, document라는 큰 놈이 있고 head가 안에 있고,그 안에 style 있고 
...
bind라는 것이 있다. 상자속에 넣은게 아니라 age라는 바구니에 34가 가리키는 주소가 들어간 것이다.

즉 age에 +1하는 것은 age binding한 값에 1 더한 것과 같음. 

34가리키던 값이 35 바뀌면 rebind라고 말한다.???



마찬가지로 위에서 me라는 곳에 object를 가리키는 주소값이 들어간다. 포인터라고 하기도 하고...

mutable은 rebind를 바꾸지 않는다. 같은 놈을 기리키되 그 놈을 바꾼다. rebind를 막는다.
...
이러면 키보드가 입력한 내용을 알아먹고, 위 코드를 통해 어떤 이벤트가 발생했는지 볼 수 있다.

어떤 킬를 눌렀는지 다 구분함.

이것은 문자마다 고유의 코드값이 있어서, 나라마다 다르게 인식시킬 필요는 없다.
