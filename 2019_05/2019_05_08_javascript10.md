...
파이어베이스 - 구글 아이디 - 프로젝트 추가 - 국가는 대한민국 - 장소는 asia/northest (south는 인도네이사에 간다)
...
samsung.com도 직접 자기 사이트 만드는게 아니라 펑타이라는 곳에 외주 맞김. 근데 communication depth가 너무 길어지니까 외주 트렌드도 점점 죽어나감.
...
처음 firebase는 (?) DB는 제멋대로. 여기서 사람들이 쓰기 불편해하니까 공통으로 쓸 언어? 를 만든 듯 하다.
...
로컬 스토리지 기억하나? 이거솓 저장소지만 브라우저마다 기억하고 있지는 않는다. 하지만 firebase는 다름.
...
그래서 rest frame work로 딱 json 파일만 넘겨보고, 그런 rest frame work server 하나를 c9에서 만들어 볼 것이다.



전달받은 파일은 로컬에 해도 될것이다.



vue js랑 django rest framework 연결
